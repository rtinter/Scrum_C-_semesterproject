...
namespace math_task_factory {
    ...
    std::unique_ptr<MathTask> createMathTask(MathTaskType type, int difficulty_level) {
        switch (type) {
            case MathTaskType::SIMPLE_MULTIPLICATION_TABLE:
                return std::make_unique<SimpleMultiplicationTable>(difficulty_level);
            case MathTaskType::EQUATION_BUILDER:
                return std::make_unique<EquationBuilder>(difficulty_level);
            default:
    ...
    std::unique_ptr<MathTask> createRandomMathTask(int difficulty_level) {
        ...
        return createMathTask(randomType, difficulty_level);
    }
}